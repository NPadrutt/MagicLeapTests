...
// ---------------------------------------------------------------------
 ...
 Shader "Magic Leap/BRDF/BRDFMultiLumaMask"
 {
	Properties
	{
	...
	SubShader
	{
		...
		void surf (Input IN, inout SurfaceOutput o)
		{
			...
			half4 masktex = tex2D (_MaskTex, IN.uv2_MaskTex);
			half3 detail = ceil(((maintex.r*masktex)+_Clamp))*-1+1;
			
			half luma1 = masktex.g*_Luma1;
			half luma2 = masktex.b*_Luma2;
			o.Albedo = lerp(_Color,maintex*(_Color*.4),_Transition);
			//o.Albedo = detail;
			o.Emission = lerp(0,(luma1+luma2)*maintex.a,_Transition);
		}
		ENDCG
	}
